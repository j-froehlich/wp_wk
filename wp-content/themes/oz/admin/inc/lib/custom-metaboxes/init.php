...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$dir = get_template_directory() . '/admin/inc/lib/custom-metaboxes';
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			$scripts[] = 'wiloke_spectrum';
			$styles[] = 'wiloke_spectrum';
			if ( ! is_admin() ) {
				wp_enqueue_script('wiloke_spectrum', WILOKE_AD_ASSET_URI . 'js/spectrum.js', array('jquery'), null, true);
            	wp_enqueue_style('wiloke_spectrum', WILOKE_AD_SOURCE_URI . 'css/spectrum.css', array(), null);
			}
		} else {
		...
		wp_register_script( 'cmb-datepicker', CMB_META_BOX_URL . 'js/jquery.datePicker.min.js', array('jquery'), null);
		wp_register_script( 'cmb-timepicker', CMB_META_BOX_URL . 'js/jquery.timePicker.min.js', array('jquery'), null);
		wp_register_script( 'cmb-scripts', CMB_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION, true );
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'oz' ),
		) ) );

		wp_register_style( 'cmb-styles', CMB_META_BOX_URL . 'style'. $min .'.css', $styles );
		wp_register_style( 'bootstrap-for-cmb', CMB_META_BOX_URL . 'css/boostrap.custom-for-metabox.min.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( $this->_meta_box['cmb_styles'] )
				...
				wp_enqueue_style( 'bootstrap-for-cmb' );
		}
	}
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		if ( isset($meta_box['additional_class']) )
		{
			$cssClass = $cssClass . ' ' . $meta_box['additional_class'];
		}
	...
	public static function wiloke_help_for_map($args)
	{
		...
		return self::wiloke_deep_sanitize_data($args, $field, $aValues);
	}
	...
	public static function nonce() {
		return get_template_directory() . '/admin/inc/lib/custom-metaboxes/init.php';
	}
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, get_template_directory_uri() . '/admin/inc/lib/custom-metaboxes' );
		  ...
		  $cmb_url = str_replace(
			    ...
			    get_template_directory_uri() . '/admin/inc/lib/custom-metaboxes'
			);
		}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'oz' ) );
