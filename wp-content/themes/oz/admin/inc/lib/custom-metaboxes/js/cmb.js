...
window.CMB = (function(window, document, $, undefined){
	...
	cmb.init = function() {
		...
		$metabox
			...
			// Visual Selection
			.on( 'click', '.visual_select label', cmb.visualSelection );
	 ...
	 * @author Wiloke
	 */
	cmb.visualSelection = function (event) {
		event.preventDefault();
		var $self = $(this);
		$self.parent().children().removeClass('checked');

		$self.parent().find('input:checked').prop('checked', false);
	...
	cmb.toggleCheckBoxes = function( event ) {
		...
		var $multicheck = $self.closest( 'td' ).find( 'input[type=checkbox]' );
	...
	cmb.handleMedia = function(event) {
		...
		var handlers = {
			...
			single : function( selection ) {
				...
				$formfield.val(attachment.url).change();
				$('#'+ cmb.formfield +'_id').val(attachment.id);
	...
	cmb.handleRemoveMedia = function( event ) {
		...
		cmb.metabox().find('input#' + cmb.formfield).val('').trigger('change');

		cmb.metabox().find('input#' + cmb.formfield + '_id').val('').trigger('change');
		if ( ! $container.length ) {
	...
	cmb.initColorPickers = function( $selector ) {
		...
		if ( typeof wilokeAdmin == 'object' || (typeof jQuery.wp === 'object' && typeof jQuery.wp.wpColorPicker === 'function')) {

			var isShowPalette = false;

			if ( WilokeAdminGlobal.ColorPalatte !== null )
			{
				isShowPalette = true;
				WilokeAdminGlobal.ColorPalatte = $.parseJSON(WilokeAdminGlobal.ColorPalatte);
			}
			...
			$selector.spectrum({
				...
				hide: function (color) {
					...
					$(this).closest('.cmb-type-colorpicker').trigger('spectrum-hide');
				},
				show: function (color) {
					if ( color !== null )
					{
					...
					$(this).closest('.cmb-type-colorpicker').trigger('spectrum-show');
					$(this).trigger('spectrum-show');
				},
				allowEmpty: true,
				showInput: true,
				showPalette: isShowPalette,
				palette: WilokeAdminGlobal.ColorPalatte
			});
    ...
    if ( $wilokeDependencyOnTemplate.length > 0 )
    {
        $wilokeDependencyOnTemplate.each(function(){
            ...
            if ( $target.length == 0 && oData.operator != 'not_contains' )
            {
    ...
    $.fn.wilokeDependency = function()
    {
        var $this   = $(this),
            oData   = $(this).data('dependency'),
            val     = '',
            status  = true,
            $target = $(this).closest('.cmb_metabox.form-table, .wiloke__box-content').find('[name="'+oData.name+'"]');

        if ( $target.hasClass('is_visual_selection') )
        {
        ...
        $target.on('click change', function (event){
            event.preventDefault();
            val = $(event.currentTarget).hasClass('js-visual-selection')  ? $(this).children().attr('value') : $(this).val();
            switch (oData.operator)
            {
                case '=':
                    if ( val ==  oData.match)
            ...
            if ( status )
            {
                ...
                {
                    // $this.closest('tr, .wiloke__filter').show();
                    $this.closest('tr, .wiloke__filter').addClass('wiloke-active');
                }
            }else{
                // $this.closest('tr, .wiloke__filter').hide();
                $this.closest('tr, .wiloke__filter').removeClass('wiloke-active');
            }
        });

        if ( $target.hasClass('js-visual-selection') )
        {
            $target.parent().find('.js-visual-selection.checked').trigger('click');
        }else{
    ...
    if ( $wilokeDependency.length > 0 )
    {
        $wilokeDependency.each(function(){
            $(this).wilokeDependency();
        });
    }


    $.fn.wilokeFormDependencyOnTemplate = function()
    {
        var $this   = $(this),
            val     = '',
    ...
    if ( $wilokeFormDependencyOnTemplate.length > 0 )
    {
        $wilokeFormDependencyOnTemplate.each(function () {
            $(this).wilokeFormDependencyOnTemplate();
        })
    }
