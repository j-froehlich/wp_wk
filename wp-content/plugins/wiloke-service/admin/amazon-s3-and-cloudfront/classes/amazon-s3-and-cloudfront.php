...
class Amazon_S3_And_CloudFront extends AWS_Plugin_Base {
	...
	function init( $plugin_file_path ) {
		...
		add_action('wp_ajax_wiloke_service_migrate_files', array($this, 'migrate_files'));
	}
	...
	public function migrate_files(){
		...
		if ( !isset($_POST['nonce']) || !check_ajax_referer(WilokeService::$nonceKey, 'nonce', false) ) {
			wp_send_json_error();
		}
		...
		if ( !empty($_POST['post__not_in']) ) {
			$args['post__not_in'] = array_map('trim', explode( ',', $_POST['post__not_in']));
		}
		...
		if ( !empty($images) && !is_wp_error($images) ) {
			...
			{
				$aPosts[$post->ID] = $post->post_title;
				$data = wp_get_attachment_metadata( $post->ID, false );
				$this->wp_update_attachment_metadata($data, $post->ID);
			}

			wp_send_json_success($aPosts);
		}
	...
	function get_settings_tabs() {
		$tabs = array(
			...
			'wiloke-migrate' => _x( 'Migration Existed Files', 'Show the media library tab', 'amazon-s3-and-cloudfront' ),
