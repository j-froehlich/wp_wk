...
$caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $el_id = $css_animation = $style = $external_style = $border_color = $css = $is_lazy_load = '';
...
switch ( $source ) {
	...
	case 'featured_image':
		...
		if ( $is_lazy_load === 'enable' ){
			$img['thumbnail']  = '<img class="vc_single_image-img lazy" data-original="'.wp_get_attachment_image_url($img_id, $img_size).'" />';
		}else{
...
if ( !$img ) {
	if ( $is_lazy_load === 'enable' ){
		$img['thumbnail'] = '<img class="vc_img-placeholder vc_single_image-img lazy" data-original="' . $default_src . '" />';
	}else{
